...
if [[ "$NAME" == "wide" ]]; then
    ...
    TRICE_PLC_LIMIT=$(awk '/Limit: r </{print 3*($4  < 1 ? 1 : $4)}'   ${2/.root/.log.PLC} | tail -n 1);
    FRAC_PLC_LIMIT=$(awk '/Limit: r </{print 0.25*$4}' ${2/.root/.log.PLC} | tail -n 1);
elif [[ "$NAME" == "exp" ]]; then
    TRICE_PLC_LIMIT=$(awk '/Limit: r </{print 3*$4}'   ${2/.root/.log.FREQ_E050} | tail -n 1);
...
TOYS=50;   MULTIPLIER=1   # each job does ($POINTS/$INTERLEAVE) * $TOYS * $MULTIPLIER toys
TOTAL_TOYS=2000          
