...
void plotFitParams_Bkg(){


  const int numMasses=3;
  vector<string> paramName;
    paramName.push_back("p1");
    paramName.push_back("p2");
    paramName.push_back("p3");
    paramName.push_back("p4");

  const int numParams=paramName.size();

  double bounds[numMasses+1]={120.,135.,150.,170.};
  
  double mass[numMasses]={127.5,142.5,160.};
  double massError[numMasses]={7.5,7.5,10.};
  ...
  for(int iMass=0; iMass<numMasses; iMass++){
    ...
    r=fitmZstarBackground(bounds[iMass],bounds[iMass+1]);
  ...
  TCanvas* cParam = new TCanvas("cParam","cParam",800,800);
  cParam->Divide(2,2);
