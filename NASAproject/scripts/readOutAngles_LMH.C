...
TLorentzVector* applyResolution(TLorentzVector *l_gen){
  ...
  if(randomForSmearing.Uniform()<.8){
    double l_Perp = l_gen->Perp()+(randomForSmearing.Gaus(0,0.025*1.15* l_gen->Perp()+0.0001*1.15* l_gen->Perp()* l_gen->Perp() ));
    double l_Theta = l_gen->Theta()+(randomForSmearing.Gaus(0,0.001));
    double l_Phi = l_gen->Phi()+(randomForSmearing.Gaus(0,0.001));
  }else{
    double l_Perp = l_gen->Perp()+randomForSmearing.Gaus(-l_gen->Perp()*.05,l_gen->Perp()*.12);
