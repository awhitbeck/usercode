...
#include "../PDFs/RooNASAModelBkg.h"
void fitmZZBkg(){
...
void plotNASABkg(){
  ...
  RooRealVar* zzmass = new RooRealVar("zzmass","zzmass",120,110,180);
  RooRealVar* D = new RooRealVar("LD","LD",0.5,0,1);
  RooRealVar* a0 = new RooRealVar("a0","a0",116.87,100,150);
  RooRealVar* a1 = new RooRealVar("a1","a1",11.65,5,15);
  RooRealVar* a2 = new RooRealVar("a2","a2",66.47,50,100);
  RooRealVar* a3 = new RooRealVar("a3","a3",0.152,0.05,0.5);
  ...
  RooNASAModelBkg* nasaPdf = new RooNASAModelBkg("nasaPDF","nasaPDF",*zzmass,*D,*a0,*a1,*a2,*a3,*a4,*a5,*a6,*a7,*a8,*a9);
  TFile *f = new TFile("Dbackground.root","READ");
  TH2F* h_mzzD= (TH2F*)(f->Get("h_mzzD"));
  cout<<"in the file "<<h_mzzD<<endl;
  cout<< "in the file " <<h_mzzD->GetBinContent(h_mzzD->FindBin(120,0.5))<<endl  ;
  nasaPdf->setD(*h_mzzD);
  
  //nasaPdf->setD(f);
  cout<<"before getval"<<endl;
  RooArgSet* argset = new RooArgSet(*zzmass,*D,*a0,*a1,*a2,*a3,*a4,*a5,*a6,"argset");
  argset->add(*a7);
  argset->add(*a8);
  argset->add(*a9);
  nasaPdf->getVal(argset);
  cout<<"after getval"<<endl;
  ...
  cout<<"finished background files"<<endl;
  RooDataSet *data = new RooDataSet("data","data",chain,RooArgSet(*zzmass),"zzmass>110 && zzmass<180");
  //mzzPdf->fitTo(*data);
  
  cout<<"roodataset defined "<<endl;
  TCanvas* cmzz = new TCanvas( "cmzz", "cmzz");
  RooPlot* mzzframe =  zzmass->frame(55);
  data->plotOn(mzzframe);
  cout<<"data defined"<<endl;
  nasaPdf->plotOn(mzzframe);
  mzzframe->Draw();
}
