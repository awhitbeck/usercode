...
#include "../PDFs/RooNASAModelZeroPlus_sigSep.h"
#include "../PDFs/RooNASAModelZeroMinus_sigSep.h"
#include "../PDFs/RooNASAModelBkg_sigSep.h"
#include "RooAddPdf.h"
#include <cmath>

class NASA2dSigSepPdfFactory{
...
public: 
  ...
  RooNASAModelZeroPlus_sigSep* zeroPlusPDF;
  RooNASAModelZeroMinus_sigSep* zeroMinusPDF;
  RooNASAModelBkg_sigSep*      bkgPDF;
  ...
  NASA2dSigSepPdfFactory(RooRealVar* mzz, RooRealVar* D,bool isZeroPlus=true){
  ...
  bkgPDF = new RooNASAModelBkg_sigSep("bkgPDF","bkgPDF",*mzz,*D,*a0,*a1,*a2,*a3,*a4,*a5,*a6,*a7,*a8,*a9);
  zeroPlusPDF = new RooNASAModelZeroPlus_sigSep("zeroPlusPDF","zeroPlusPDF",*mzz,*D,*mean,*sigma,*mean2,*sigma2,*frac);
  zeroMinusPDF = new RooNASAModelZeroMinus_sigSep("zeroMinusPDF","zeroMinusPDF",*mzz,*D,*mean,*sigma,*mean2,*sigma2,*frac);

  nsig = new RooRealVar("nsig","nsig",0.,1000.);
  nbkg = new RooRealVar("nbkg","nbkg",0.,1000.);

  if(isZeroPlus)
    PDF = new RooAddPdf("PDF","PDF",RooArgList(*zeroPlusPDF,*bkgPDF),RooArgList(*nsig,*nbkg));
  else
    PDF = new RooAddPdf("PDF","PDF",RooArgList(*zeroMinusPDF,*bkgPDF),RooArgList(*nsig,*nbkg));
  };

  ~NASA2dSigSepPdfFactory(){
