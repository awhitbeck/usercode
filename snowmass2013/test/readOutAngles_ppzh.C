...
void readOutAngles_ppzh(std::string filename, bool applyAcc=false, bool debug = false ) {
    ...
    tree->Branch("rand"         , &m_rand, "rand/I");
	...
	int rand_0or1 = rand()%2;

	m_rand = rand_0or1; 
	
	if ( debug ) {
	  std::cout << "rand_0or1 = " << rand_0or1 << "\n";
	}
	
	// do a random flip of the Z coordinates
	if ( rand_0or1 == 0 ) {
	  pZstar.SetPxPyPzE(pZstar.Py(), -pZstar.Px(), -pZstar.Pz(), pZstar.E());	  
	  pH.SetPxPyPzE(pH.Py(), -pH.Px(), -pH.Pz(), pH.E());
	  pZ.SetPxPyPzE(pZ.Py(), -pZ.Px(), -pZ.Pz(), pZ.E());
	  p_lplus.SetPxPyPzE(p_lplus.Py(), -p_lplus.Px(), -p_lplus.Pz(), p_lplus.E());
	  p_lminus.SetPxPyPzE(p_lminus.Py(), -p_lminus.Px(), -p_lminus.Pz(), p_lminus.E());
	  p_b.SetPxPyPzE(p_b.Py(), -p_b.Px(), -p_b.Pz(), p_b.E());
	  p_bbar.SetPxPyPzE(p_bbar.Py(), -p_bbar.Px(), -p_bbar.Pz(), p_bbar.E());
	}
