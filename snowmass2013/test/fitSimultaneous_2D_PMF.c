...
int execute_fitSimultaneous_2D (sample smp, const int erg_tev, const double fixval_fg2=-1, const double fixval_fg4=-1, const int iteration=0, int iteration_oldfirstevent=0, const int event_selection=0, const int ntests=200, const int nevents=600){
	...
	string OUTPUT_NAME = "2DFits/ggtoHtoZZ4l_125_6_PseudoExp_PMF_";
	if(nevents!=600){
		char cnevents[20];
	...
	TFile* finput[6];
	TH1F* h1D[6*(kFitTotalD-2)]; // "0+m","0+h","0-","0+m0+h","0+m0-","0+h0-"
	TH2F* h2D[6*kFitTotalD/2];
	for(int f=0;f<6;f++){
		string cinput = cinput_common + INPUT_NAME + sampleName[f] + ".root";
		finput[f] = new TFile(cinput.c_str(),"read");

		for(int t=0;t<(kFitTotalD-2)/2;t++){
			string templatename = str1DFit_title[t];
		...
		for(int t=kFitTotalD/2;t<kFitTotalD-1;t++){
			string templatename = str1DFit_title[t];
			if(event_selection!=0) templatename = templatename + "_" + idealdetector_cut_title[event_selection];
			h1D[6*(t-1)+f] = (TH1F*) finput[f]->Get(templatename.c_str());
			*h1D[6*(t-1)+f] = *h1D[6*(t-1)+f] * (sample_xsec_ratio[f]);
		};
		for(int t=0;t<kFitTotalD/2;t++){
			string templatename = str2DFit_title[t];
			if(event_selection!=0) templatename = templatename + "_" + idealdetector_cut_title[event_selection];
	...
	for(int t=0;t<kFitTotalD;t++){
		if(t==kFitTotalD/2-1 || t==kFitTotalD-1) continue;
...
//			foutput->WriteTObject(h2D[6*t+f]);
	...
	for(int t=0;t<(kFitTotalD-2);t++){
		...
		if(t<(kFitTotalD-2)/2){
			str1Dtree = str1Dtree + str1DFit_title[t];
		}
		else{
			str1Dtree = str1Dtree + str1DFit_title[t+1];
		};

		kd1_name = strFitDim[t];
		if(t==(kFitTotalD/2-1)) kd1_name = strFitDim[0];
		kd12_name = strFitDim[t+1];
		if(t<(kFitTotalD/2-1)) kd2_name = strFitDim[t+kFitTotalD/2];
		else if(t==(kFitTotalD/2-1)) kd2_name = strFitDim[1];

		RooRealVar* kD1;
		RooRealVar* kD12;
		RooRealVar* kD2;
		if( t<(kFitTotalD/2) ) kD1 = new RooRealVar(kd1_name.c_str(),kd1_name.c_str(),0.0,1.0);
		else kD1 = new RooRealVar(kd1_name.c_str(),kd1_name.c_str(),-1.0,1.0);
		if( t<(kFitTotalD/2-1) ) kD12 = new RooRealVar(kd12_name.c_str(),kd12_name.c_str(),0.0,1.0);
		else kD12 = new RooRealVar(kd12_name.c_str(),kd12_name.c_str(),-1.0,1.0);
		if( t<(kFitTotalD/2-1) ) kD2 = new RooRealVar(kd2_name.c_str(),kd2_name.c_str(),-1.0,1.0);
		else if( t==(kFitTotalD/2-1) ) kD2 = new RooRealVar(kd2_name.c_str(),kd2_name.c_str(),0.0,1.0);
...
*/
		...
		if( t<(kFitTotalD/2-1) ) pdf_1D = new RooSpinZero_KD_ZH_Simultaneous_PMF("pdf_1D","pdf_1D",*kD1,*f_g2,*f_g4,histos_1D,false,fix_fg2,fix_fg4);
		else pdf_1D = new RooSpinZero_KD_ZH_Simultaneous_PMF("pdf_1D","pdf_1D",*kD12,*f_g2,*f_g4,histos_1D,false,fix_fg2,fix_fg4);
		...
		if(t<kFitTotalD/2){
			str2Dtree = str2Dtree + str2DFit_title[t];
...
*/
			...
			if( t<(kFitTotalD/2-1) ) toy_data_1D = new RooDataSet("toy_data_1D","toy_data_1D", toy_tree[test], RooArgSet(*kD1));
			else toy_data_1D = new RooDataSet("toy_data_1D","toy_data_1D", toy_tree[test], RooArgSet(*kD12));
			RooFitResult* sum_res_pdf1D = pdf_1D->fitTo(*toy_data_1D,Save(true),PrintLevel(-1));
			minLL = sum_res_pdf1D->minNll();
			minLL /= nevents;

//			if( t<(kFitTotalD/2-1) ) cout << "Embedded Toy " << test << "in 1D: " << kD1->GetName() << endl;
...
void fitSimultaneous_2D_PMF (sample smp, const int erg_tev, const int iteration_min=0, const int iteration_max=5, const int event_selection=0, const int ntests=200, const int nevents=600){
